...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            ret = '<|start_header_id|>'+'SYSTEM'+'<|end_header_id|>'+self.sep+self.system+'<|eot_id|>'
            for role, message in messages:
                if message:
                    ret += '<|start_header_id|>'+role+'<|end_header_id|>'+self.sep+message+'<|eot_id|>'
                else:
                    ret += '<|start_header_id|>'+role+'<|end_header_id|>'+self.sep
...
conv_llama_3 = Conversation(
    system ="You are an AI assistant tasked with modeling the evolution of a temporal graph. "
    "Your goal is to analyze the temporal dependencies among the entities and provide answers to the user's question.",
    roles=("USER", "ASSISTANT"),
    version="llama3",
    messages=(),
    offset=0,
