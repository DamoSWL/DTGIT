...
#     def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler=None):
...
#         self.freq_feat_dim = 50
#         self.channel_embedding_dim = 50
#         self.patch_size = 1
#         self.num_layers = 2
#         self.num_heads = 2
#         self.dropout = 0.1
#         self.max_input_sequence_length = 48
#         self.num_channels = 4
#         self.hidden_size = self.node_feat_dim

#         self.time_encoder = TimeEncoder(time_dim=self.time_feat_dim)

#         self.neighbor_co_occurrence_encoder = NeighborCooccurrenceEncoder(self.freq_feat_dim,neighbor_sampler=neighbor_sampler)


#         self.encoder_layer = nn.ModuleDict({
...
#             'neighbor_co_occurrence': nn.Linear(in_features=self.patch_size * self.freq_feat_dim, out_features=self.channel_embedding_dim, bias=True)
#         })
...
#     def set_max_input_sequence_length(self,max_input_sequence_length:int):
#         self.max_input_sequence_length = max_input_sequence_length

#     def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, node_interact_times: np.ndarray):
#         """
...
#         #     self.neighbor_co_occurrence_encoder(src_nodes_neighbor_ids_list,dst_nodes_neighbor_ids_list,
...
#         src_padded_nodes_neighbor_node_raw_features = self.encoder_layer['node'](src_padded_nodes_neighbor_node_raw_features)
#         src_padded_nodes_edge_raw_features = self.encoder_layer['edge'](src_padded_nodes_edge_raw_features)
#         src_padded_nodes_neighbor_time_features = self.encoder_layer['time'](src_padded_nodes_neighbor_time_features)
#         src_padded_nodes_neighbor_co_occurrence_features = self.encoder_layer['neighbor_co_occurrence'](src_padded_nodes_neighbor_co_occurrence_features)
...
#                         src_patches_nodes_neighbor_time_features,src_patches_nodes_neighbor_co_occurrence_features]
#         src_patches_data = torch.cat(src_patches_data, dim=-1)
        
#         # dst_patches_data = [dst_patches_nodes_neighbor_node_raw_features, dst_patches_nodes_edge_raw_features,
#         #                 dst_patches_nodes_neighbor_time_features,dst_patches_nodes_neighbor_co_occurrence_features]
#         # dst_patches_data = torch.cat(dst_patches_data, dim=-1)
...
#         for transformer in self.transformers:
#             src_patches_data = transformer(src_patches_data,neighbor_masks=src_padded_nodes_neighbor_ids)
#             # dst_patches_data = transformer(dst_patches_data,neighbor_masks=dst_padded_nodes_neighbor_ids)
...
#         # src_patches_data = src_patches_data[:,0,:]
...
#                      padded_nodes_neighbor_times: np.ndarray, time_encoder):
#         """
...
#         # patches_nodes_neighbor_node_raw_features = torch.stack(patches_nodes_neighbor_node_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.channel_embedding_dim)
#         # # Tensor, shape (batch_size, num_patches, patch_size * edge_feat_dim)
#         # patches_nodes_edge_raw_features = torch.stack(patches_nodes_edge_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.channel_embedding_dim)
#         # # Tensor, shape (batch_size, num_patches, patch_size * time_feat_dim)
#         # patches_nodes_neighbor_time_features = torch.stack(patches_nodes_neighbor_time_features, dim=1).reshape(batch_size, num_patches, patch_size * self.channel_embedding_dim)

#         # patches_nodes_neighbor_co_occurrence_features = torch.stack(patches_nodes_neighbor_co_occurrence_features, dim=1).reshape(batch_size, num_patches, patch_size * self.channel_embedding_dim)
...
#         return padded_nodes_neighbor_node_raw_features, padded_nodes_edge_raw_features, padded_nodes_neighbor_time_features, padded_nodes_neighbor_co_occurrence_features
...
#     def __init__(self, neighbor_co_occurrence_feat_dim: int, neighbor_sampler):
#         """
...
#         self.neighbor_co_occurrence_encode_layer = FrequencyEncoder(fre_dim=self.neighbor_co_occurrence_feat_dim,parameter_requires_grad=True)
...
#         for src_padded_node_neighbor_id in src_padded_nodes_neighbor_ids:
...
#             src_unique_keys, src_inverse_indices, src_counts = np.unique(src_padded_node_neighbor_id, return_inverse=True, return_counts=True)
...
#             # dst_padded_node_neighbor_counts_in_dst = torch.from_numpy(dst_counts[dst_inverse_indices]).float().to(self.neighbor_co_occurrence_encode_layer.w.device)
...
#             # Tensor, shape (src_max_seq_length, )
#             # src_padded_node_neighbor_counts_in_dst = torch.from_numpy(src_padded_node_neighbor_id.copy()).apply_(lambda neighbor_id: dst_mapping_dict.get(neighbor_id, 0.0)).float().to(self.neighbor_co_occurrence_encode_layer.w.device)
#             # Tensor, shape (src_max_seq_length, 2)
#             src_padded_nodes_appearances.append(src_padded_node_neighbor_counts_in_src)

#             # we need to use copy() to avoid the modification of dst_padded_node_neighbor_ids
#             # Tensor, shape (dst_max_seq_length, )
#             # dst_padded_node_neighbor_counts_in_src = torch.from_numpy(dst_padded_node_neighbor_ids.copy()).apply_(lambda neighbor_id: src_mapping_dict.get(neighbor_id, 0.0)).float().to(self.neighbor_co_occurrence_encode_layer.w.device)
...
#         src_padded_nodes_appearances = src_padded_nodes_appearances.unsqueeze(dim=-1)
...
#         return src_padded_nodes_appearances

#     # def count_nodes_appearances2(self,src_nodes_neighbor_ids_list,dst_nodes_neighbor_ids_list,src_padded_nodes_neighbor_ids,dst_padded_nodes_neighbor_ids):

#     #     src_padded_nodes_appearances_src = torch.from_numpy(src_padded_nodes_neighbor_ids.copy())
#     #     src_padded_nodes_appearances_dst = torch.from_numpy(src_padded_nodes_neighbor_ids.copy())
#     #     dst_padded_nodes_appearances_src = torch.from_numpy(dst_padded_nodes_neighbor_ids.copy())
#     #     dst_padded_nodes_appearances_dst = torch.from_numpy(dst_padded_nodes_neighbor_ids.copy())

#     #     for i in range(src_padded_nodes_neighbor_ids.shape[0]):

#     #         src_node_id = src_padded_nodes_neighbor_ids[i][0]   
#     #         src_neighbors_counter = Counter(src_nodes_neighbor_ids_list[i]) 
#     #         src_neighbors_counter[src_node_id] = 1
#     #         src_padded_nodes_appearances_src[i].apply_(lambda neighbor_id: src_neighbors_counter.get(neighbor_id, 0))
#     #         dst_padded_nodes_appearances_src[i].apply_(lambda neighbor_id: src_neighbors_counter.get(neighbor_id, 0))
        
#     #         dst_node_id = dst_padded_nodes_neighbor_ids[i][0]   
#     #         dst_neighbors_counter = Counter(dst_nodes_neighbor_ids_list[i]) 
#     #         dst_neighbors_counter[dst_node_id] = 1
#     #         dst_padded_nodes_appearances_dst[i].apply_(lambda neighbor_id: dst_neighbors_counter.get(neighbor_id, 0))
#     #         src_padded_nodes_appearances_dst[i].apply_(lambda neighbor_id: dst_neighbors_counter.get(neighbor_id, 0))


#     #     src_padded_nodes_appearances_src = src_padded_nodes_appearances_src.float().to(self.neighbor_co_occurrence_encode_layer.w.device)
#     #     src_padded_nodes_appearances_dst = src_padded_nodes_appearances_dst.float().to(self.neighbor_co_occurrence_encode_layer.w.device)
#     #     dst_padded_nodes_appearances_src = dst_padded_nodes_appearances_src.float().to(self.neighbor_co_occurrence_encode_layer.w.device)
#     #     dst_padded_nodes_appearances_dst = dst_padded_nodes_appearances_dst.float().to(self.neighbor_co_occurrence_encode_layer.w.device)

#     #     src_padded_nodes_appearances_src /= (src_padded_nodes_appearances_src.sum(dim=-1,keepdim=True) + 1e-5)
#     #     src_padded_nodes_appearances_dst /= (src_padded_nodes_appearances_dst.sum(dim=-1,keepdim=True) + 1e-5)
#     #     dst_padded_nodes_appearances_src /= (dst_padded_nodes_appearances_src.sum(dim=-1,keepdim=True) + 1e-5)
#     #     dst_padded_nodes_appearances_dst /= (dst_padded_nodes_appearances_dst.sum(dim=-1,keepdim=True) + 1e-5)


#     #     # src_padded_nodes_appearances = src_padded_nodes_appearances.unsqueeze(dim=-1)
#     #     # dst_padded_nodes_appearances = dst_padded_nodes_appearances.unsqueeze(dim=-1)

#     #     return src_padded_nodes_appearances_src, src_padded_nodes_appearances_dst,dst_padded_nodes_appearances_src,dst_padded_nodes_appearances_dst
...
#         src_padded_nodes_appearances = self.count_nodes_appearances(src_padded_nodes_neighbor_ids=src_padded_nodes_neighbor_ids)
...
#         frequency = src_padded_nodes_appearances[:,:,0]/(src_padded_nodes_appearances[:,:,0].sum(dim=-1,keepdim=True))
       
#         src_padded_nodes_neighbor_co_occurrence_features_1 = self.neighbor_co_occurrence_encode_layer(frequency)
...
#         # src_padded_nodes_neighbor_co_occurrence_features, Tensor, shape (batch_size, src_max_seq_length, neighbor_co_occurrence_feat_dim)
#         # dst_padded_nodes_neighbor_co_occurrence_features, Tensor, shape (batch_size, dst_max_seq_length, neighbor_co_occurrence_feat_dim)
#         return src_padded_nodes_neighbor_co_occurrence_features


#     # def forward(self,src_nodes_neighbor_ids_list,dst_nodes_neighbor_ids_list,src_padded_nodes_neighbor_ids,dst_padded_nodes_neighbor_ids):
#     #     src_padded_nodes_appearances_src, src_padded_nodes_appearances_dst,dst_padded_nodes_appearances_src,dst_padded_nodes_appearances_dst = \
#     #                                                 self.count_nodes_appearances2(src_nodes_neighbor_ids_list,dst_nodes_neighbor_ids_list,
#     #                                                                                               src_padded_nodes_neighbor_ids,dst_padded_nodes_neighbor_ids)
      
#     #     src_padded_nodes_neighbor_co_occurrence_src_features = self.neighbor_co_occurrence_encode_layer(src_padded_nodes_appearances_src)
#     #     src_padded_nodes_neighbor_co_occurrence_dst_features = self.neighbor_co_occurrence_encode_layer(src_padded_nodes_appearances_dst)
#     #     dst_padded_nodes_neighbor_co_occurrence_src_features = self.neighbor_co_occurrence_encode_layer(dst_padded_nodes_appearances_src)
#     #     dst_padded_nodes_neighbor_co_occurrence_dst_features = self.neighbor_co_occurrence_encode_layer(dst_padded_nodes_appearances_dst)

#     #     src_padded_nodes_neighbor_co_occurrence_src_features[src_padded_nodes_neighbor_ids == 0] = 0
#     #     src_padded_nodes_neighbor_co_occurrence_dst_features[src_padded_nodes_neighbor_ids == 0] = 0

#     #     dst_padded_nodes_neighbor_co_occurrence_src_features[dst_padded_nodes_neighbor_ids == 0] = 0
#     #     dst_padded_nodes_neighbor_co_occurrence_dst_features[dst_padded_nodes_neighbor_ids == 0] = 0   

#     #     src_padded_nodes_neighbor_co_occurrence_features = src_padded_nodes_neighbor_co_occurrence_src_features + src_padded_nodes_neighbor_co_occurrence_dst_features
#     #     dst_padded_nodes_neighbor_co_occurrence_features = dst_padded_nodes_neighbor_co_occurrence_src_features + dst_padded_nodes_neighbor_co_occurrence_dst_features
...
#             nn.ReLU(),
#             # nn.Linear(in_features=self.fre_dim, out_features=self.fre_dim//2),
#             # nn.Linear(in_features=self.fre_dim, out_features=self.fre_dim//2)
#             )
...
#         frequency = frequency.to(self.w[0].weight.dtype)
#         frequency = frequency.to(self.w[0].weight.device)
        

#         # Tensor, shape (batch_size, seq_len, time_dim)
#         # output = torch.sin(frequency * self.w.expand(frequency.shape[1],-1))
...
#         :param dropout: float, dropout rate
#         """
#         super(MLPMixer_part, self).__init__()
...
#         hidden_tensor = self.channel_norm(input_tensor)
#         # Tensor, shape (batch_size, num_tokens, num_channels)
...
class TemporalGraph(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        # Tensor, shape (batch_size, dst_num_patches, channel_embedding_dim)
        dst_padded_nodes_neighbor_node_raw_features = self.encoder_layer['node'](dst_padded_nodes_neighbor_node_raw_features)
        dst_padded_nodes_edge_raw_features = self.encoder_layer['edge'](dst_padded_nodes_edge_raw_features)
        dst_padded_nodes_neighbor_time_features = self.encoder_layer['time'](dst_padded_nodes_neighbor_time_features)
        dst_padded_nodes_neighbor_co_occurrence_features = self.encoder_layer['neighbor_co_occurrence'](dst_padded_nodes_neighbor_co_occurrence_features)
        ...
        # src_patches_data = src_patches_data[:,0,:]
        # dst_patches_data = dst_patches_data[:,0,:]
        ...
        return src_node_embeddings, dst_node_embeddings, torch.from_numpy(src_padded_nodes_neighbor_ids), torch.from_numpy(dst_padded_nodes_neighbor_ids)
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
        ...
        # include the target node itself
        max_seq_length = self.max_input_sequence_length-1
        max_seq_length += 1
        if max_seq_length % patch_size != 0:
            max_seq_length += (patch_size - max_seq_length % patch_size)
        assert max_seq_length % patch_size == 0
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray,dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        src_padded_nodes_appearances = src_padded_nodes_appearances.sum(dim=-1)
        dst_padded_nodes_appearances = dst_padded_nodes_appearances.sum(dim=-1)

        src_frequency = src_padded_nodes_appearances.float()/(src_padded_nodes_appearances.sum(dim=-1,keepdim=True))
        dst_frequency = dst_padded_nodes_appearances.float()/(dst_padded_nodes_appearances.sum(dim=-1,keepdim=True))
        
        src_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer(src_frequency)
        dst_padded_nodes_neighbor_co_occurrence_features = self.neighbor_co_occurrence_encode_layer(dst_frequency)
...
class FrequencyEncoder(nn.Module):
    ...
    def __init__(self, fre_dim: int, parameter_requires_grad: bool = True):
        ...
        # self.w = nn.Linear(1, fre_dim)
        # self.w.weight = nn.Parameter((torch.from_numpy(2*math.pi * np.linspace(0, fre_dim, fre_dim, dtype=np.float32))).reshape(fre_dim, -1))
        # self.w.bias = nn.Parameter(torch.zeros(fre_dim))
        self.w = nn.Sequential(
            nn.Linear(in_features=1, out_features=self.fre_dim),
            nn.ReLU()
            # nn.Linear(in_features=self.fre_dim, out_features=self.fre_dim//2),
        ...
        # w2 = torch.tensor(2*math.pi * np.linspace(0, fre_dim, fre_dim)).unsqueeze(dim=0)
    ...
    def forward(self, frequency: torch.Tensor):
        ...
        # Tensor, shape (batch_size, seq_len, time_dim)
        # frequency_1 = torch.cos(frequency * self.w2.expand(frequency.shape[1],-1))
        # frequency_2 = torch.sin(frequency * self.w2.expand(frequency.shape[1],-1))
        # frequency_3 = frequency_1 + frequency_2
        # frequency_3 = torch.cat((frequency_1,frequency_2),dim=-1)
        output = self.w(frequency)
