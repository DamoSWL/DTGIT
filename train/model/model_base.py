...
class TemporalGraph(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler=None):
        ...
        self.neighbor_co_occurrence_encoder = NeighborCooccurrenceEncoder(self.max_input_sequence_length-1,self.freq_feat_dim,neighbor_sampler=neighbor_sampler)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, no_cross_flag=False):
        """
        ...
        for i,transformer in enumerate(self.transformers):
            ...
            src_patches_data = transformer(inputs_query=src_patches_data, inputs_key=src_patches_data,
                                            inputs_value=src_patches_data, neighbor_masks=src_padded_nodes_neighbor_ids)
            # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
            dst_patches_data = transformer(inputs_query=dst_patches_data, inputs_key=dst_patches_data,
                                            inputs_value=dst_patches_data, neighbor_masks=dst_padded_nodes_neighbor_ids)
            
            if not no_cross_flag:
                # cross-attention block
                # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
                src_node_features = transformer(inputs_query=src_patches_data, inputs_key=dst_patches_data,
                                                inputs_value=dst_patches_data, neighbor_masks=dst_padded_nodes_neighbor_ids)
                # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
                dst_node_features = transformer(inputs_query=dst_patches_data, inputs_key=src_patches_data,
                                                inputs_value=src_patches_data, neighbor_masks=src_padded_nodes_neighbor_ids)

                src_patches_data, dst_patches_data = src_node_features, dst_node_features
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def __init__(self, num_neighbors:int, neighbor_co_occurrence_feat_dim: int, neighbor_sampler):
        """
    ...
    def forward(self, src_padded_nodes_neighbor_ids: np.ndarray,dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        # src_padded_nodes_appearances = src_padded_nodes_appearances.to(self.neighbor_co_occurrence_encode_layer[0].weight.device)
        # dst_padded_nodes_appearances = dst_padded_nodes_appearances.to(self.neighbor_co_occurrence_encode_layer[0].weight.device)
...
class FrequencyEncoder(nn.Module):
    ...
    def __init__(self, fre_dim: int, parameter_requires_grad: bool = True):
        ...
        w2 = torch.tensor(2*math.pi*np.arange(1, fre_dim+1, dtype=np.float32)).unsqueeze(dim=0)
